...
# for column in schedules.columns:
#     if column == 'spread_covered' or column == 'spread_covered_by' or column == 'over_under_covered':
#         continue
    
#     # schedules.plot.scatter(x='points_scored', y=column)
#     correlation = schedules['over_under_covered'].corr(schedules[column])
...
# get all over_under data
over_under_data = get_over_under_data(2023, 2023)

over_under_data.drop(['home_team', 'away_team', 'over_under', 'spread', 'over_under_result', 'result_total_points_scored', 'schedule_id'], axis=1, inplace=True)

for column in over_under_data.columns:
    if column == 'spread_covered' or column == 'over_under_covered_by' or column == 'over_under_covered' or column == 'total_points_scored':
        continue
    
    # over_under_data.plot.scatter(x='over_under_covered', y=column)
    correlation = over_under_data['over_under_covered'].corr(over_under_data[column])
    # plt.title(f"Correlation: {correlation}")

    # Calculate the line of best fit
    # m, b = np.polyfit(over_under_data['over_under_covered'], over_under_data[column], 1)
    # plt.plot(over_under_data['over_under_covered'], m * over_under_data['over_under_covered'] + b, color='red')
