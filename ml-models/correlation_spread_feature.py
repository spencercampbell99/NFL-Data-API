...
for column in schedules.columns[:len(schedules.columns)//2]:
    if column == 'spread_covered' or column == 'spread_covered_by':
        continue
    
    # schedules.plot.scatter(x='points_scored', y=column)
    correlation = schedules['spread_covered_by'].corr(schedules[column])
