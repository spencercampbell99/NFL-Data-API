...
WITH GameData AS (
    ...
    SELECT
        ...
        SUM(CASE WHEN correct_winner_by_score = true THEN 1 ELSE 0 END) AS correct_winner_count_by_score,
        ...
        SUM(CASE WHEN correct_winner_by_score = true THEN 1 ELSE 0 END) / COUNT(*) * 100 AS overall_correct_winner_rate_by_score,
        ...
        SUM(CASE WHEN (:teams IS NULL OR home_team_char_id IN (:teams)) AND correct_winner_by_score = true AND home_win = true THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN (:teams IS NULL OR home_team_char_id IN (:teams)) AND predicted_home_win = true THEN 1 ELSE 0 END), 0) * 100 AS home_correct_winner_rate_by_score,
        SUM(CASE WHEN (:teams IS NULL OR away_team_char_id IN (:teams)) AND correct_winner_by_score = true AND home_win = false THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN (:teams IS NULL OR away_team_char_id IN (:teams)) AND predicted_home_win = false THEN 1 ELSE 0 END), 0) * 100 AS away_correct_winner_rate_by_score,
