...
WITH GameData AS (
    SELECT
        ...
        mp.home_team_score = mp.away_team_score AS predicted_tie_by_score,
        sch.home_score,
        sch.away_score,
        mp.correct_winner,
        mp.correct_winner_by_score,
        mp.correct_spread,
        mp.correct_over_under,
        mp.correct_underdog_win,
        mp.correct_underdog_win_by_score,
        mp.suggested_moneyline_percent_bet as kelly_bet,
        mp.suggested_moneyline_percent_bet_by_score as kelly_bet_by_score,
        sch.home_moneyline,
        sch.away_moneyline,
        sch.spread,
        ...
        CASE
            WHEN mp.home_team_score > mp.away_team_score AND sch.home_moneyline < 0 THEN 100 / (ABS(sch.home_moneyline)) + 1
            WHEN mp.home_team_score < mp.away_team_score AND sch.away_moneyline < 0 THEN 100 / (ABS(sch.away_moneyline)) + 1
            WHEN mp.home_team_score > mp.away_team_score AND sch.home_moneyline > 0 THEN (sch.home_moneyline / 100) + 1
            WHEN mp.home_team_score < mp.away_team_score AND sch.away_moneyline > 0 THEN (sch.away_moneyline / 100) + 1
        END AS decimal_odds_score_model,
        CASE
            WHEN mp.home_win AND sch.home_moneyline < 0 THEN 100 / (ABS(sch.home_moneyline)) + 1
            WHEN NOT mp.home_win AND sch.away_moneyline < 0 THEN 100 / (ABS(sch.away_moneyline)) + 1
            WHEN mp.home_win AND sch.home_moneyline > 0 THEN (sch.home_moneyline / 100) + 1
            WHEN NOT mp.home_win AND sch.away_moneyline > 0 THEN (sch.away_moneyline / 100) + 1
        END AS decimal_odds
    FROM
    ...
    SELECT
        ...
        SUM(CASE WHEN correct_winner_by_score = true AND NOT predicted_tie_by_score THEN 1 ELSE 0 END) AS correct_winner_count_by_score,
        ...
        SUM(CASE WHEN correct_winner_by_score = true AND NOT predicted_tie_by_score THEN 1 ELSE 0 END) / NULLIF(COUNT(CASE WHEN NOT predicted_tie_by_score THEN 1 END), 0) * 100 AS overall_correct_winner_rate_by_score,
        ...
        SUM(CASE WHEN (:teams IS NULL OR home_team_char_id IN (:teams)) AND correct_winner_by_score = true AND NOT predicted_tie_by_score AND home_win = true THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN (:teams IS NULL OR home_team_char_id IN (:teams)) AND predicted_home_win = true THEN 1 ELSE 0 END), 0) * 100 AS home_correct_winner_rate_by_score,
        SUM(CASE WHEN (:teams IS NULL OR away_team_char_id IN (:teams)) AND correct_winner_by_score = true AND NOT predicted_tie_by_score AND home_win = false THEN 1 ELSE 0 END) / NULLIF(SUM(CASE WHEN (:teams IS NULL OR away_team_char_id IN (:teams)) AND predicted_home_win = false AND NOT predicted_tie_by_score THEN 1 ELSE 0 END), 0) * 100 AS away_correct_winner_rate_by_score,
        SUM(CASE WHEN correct_winner = true THEN decimal_odds * 1 ELSE 0 END) / (COUNT(*) * 1) * 100 - 100 AS total_return_rate_moneyline,
        SUM(CASE WHEN correct_winner_by_score = true THEN decimal_odds_score_model ELSE 0 END) / (COUNT(CASE WHEN predicted_tie_by_score THEN null ELSE 1 END) * 1) * 100 - 100 AS total_return_rate_moneyline_by_score,
...
SELECT
    ...
    s.correct_winner_count_by_score,
    ...
    s.overall_correct_winner_rate_by_score,
    s.home_correct_winner_rate,
    s.away_correct_winner_rate,
    s.home_correct_winner_rate_by_score,
    s.away_correct_winner_rate_by_score,
    s.total_return_rate_moneyline,
    s.total_return_rate_moneyline_by_score,
    s.total_return_over_under,
    s.total_return_spread,
    GROUP_CONCAT(r.week ORDER BY r.week) AS weeks,
    GROUP_CONCAT(r.roi_per_week_moneyline_by_score ORDER BY r.week) AS roi_per_week_moneyline_by_score,
    GROUP_CONCAT(r.roi_per_week_moneyline ORDER BY r.week) AS roi_per_week_moneyline,
    GROUP_CONCAT(r.roi_per_week_kelly ORDER BY r.week) AS roi_per_week_kelly,
    GROUP_CONCAT(r.roi_per_week_kelly_by_score ORDER BY r.week) AS roi_per_week_kelly_by_score,
