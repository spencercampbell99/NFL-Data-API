...
agg_home AS (
    SELECT
        SUM(passing_yards) AS passing_yards_home,
        ...
        SUM(passing_touchdowns) AS passing_tds_home,
        SUM(passing_interceptions) AS interceptions_home,
...
agg_away AS (
    SELECT
        SUM(passing_yards) AS passing_yards_away,
        ...
        SUM(passing_touchdowns) AS passing_tds_away,
        SUM(passing_interceptions) AS interceptions_away,
...
agg_division AS (
    SELECT
        SUM(passing_yards) AS passing_yards_division,
        ...
        SUM(passing_touchdowns) AS passing_tds_division,
        SUM(passing_interceptions) AS interceptions_division,
...
calc_agg AS (
    SELECT
        ...
        ROUND(total_completions / total_attempts, 1) AS completion_rate,
        ...
        ROUND(passing_epa, 1) AS passing_epa,
        ROUND(passing_yards, 1) AS passing_yards,
        ROUND(passing_tds, 1) AS passing_tds,
        ROUND(interceptions, 1) AS interceptions
    FROM
        agg
),
calc_agg_home AS (
    SELECT
        ROUND(
            (
                ...
                LEAST(GREATEST((passing_yards_home / total_attempts_home - 3) * 0.25, 0), 2.375) +
                LEAST(GREATEST((passing_tds_home / total_attempts_home) * 20, 0), 2.375) +
                LEAST(GREATEST(2.375 - (interceptions_home * 25) / total_attempts_home, 0), 2.375)
            ) / 6 * 100, 1) AS passer_rating_home,
        ROUND(passing_yards_home / total_attempts_home, 1) AS yards_per_attempt_home,
        ROUND(passing_yards_home / total_completions_home, 1) AS yards_per_completion_home,
        ROUND(passing_yards_home, 1) AS passing_yards_home,
        ...
        ROUND(passing_tds_home / total_attempts_home, 1) AS td_rate_home,
        ROUND(total_completions_home / total_attempts_home, 1) AS completion_rate_home,
        ROUND((passing_yards_home - total_air_yards_home) / NULLIF(total_completions_home, 0), 1) AS yards_after_catch_per_completion_home
    FROM
        agg_home
),
calc_agg_away AS (
    SELECT
        ROUND(
            (
                ...
                LEAST(GREATEST((passing_yards_away / total_attempts_away - 3) * 0.25, 0), 2.375) +
                LEAST(GREATEST((passing_tds_away / total_attempts_away) * 20, 0), 2.375) +
                LEAST(GREATEST(2.375 - (interceptions_away * 25) / total_attempts_away, 0), 2.375)
            ) / 6 * 100, 1) AS passer_rating_away,
        ROUND(passing_yards_away / total_attempts_away, 1) AS yards_per_attempt_away,
        ROUND(passing_yards_away / total_completions_away, 1) AS yards_per_completion_away,
        ROUND(passing_yards_away, 1) AS passing_yards_away,
        ...
        ROUND(passing_tds_away / total_attempts_away, 1) AS td_rate_away,
        ROUND(total_completions_away / total_attempts_away, 1) AS completion_rate_away,
        ROUND((passing_yards_away - total_air_yards_away) / NULLIF(total_completions_away, 0), 1) AS yards_after_catch_per_completion_away
    FROM
        agg_away
),
calc_agg_division AS (
    SELECT
        ...
        (
            ...
            LEAST(GREATEST((passing_yards_division / total_attempts_division - 3) * 0.25, 0), 2.375) +
            LEAST(GREATEST((passing_tds_division / total_attempts_division) * 20, 0), 2.375) +
            LEAST(GREATEST(2.375 - (interceptions_division * 25) / total_attempts_division, 0), 2.375)
            ) / 6 * 100, 1
        ) AS passer_rating_division,
        ROUND(passing_yards_division / total_attempts_division, 1) AS yards_per_attempt_division,
        ROUND(passing_yards_division / total_completions_division, 1) AS yards_per_completion_division,
        ROUND(passing_yards_division, 1) AS passing_yards_division,
        ...
        ROUND(passing_tds_division / total_attempts_division, 1) AS td_rate_division,
        ROUND(total_completions_division / total_attempts_division, 1) AS completion_rate_division,
        ROUND((passing_yards_division - total_air_yards_division) / NULLIF(total_completions_division, 0), 1) AS yards_after_catch_per_completion_division
    FROM
        agg_division
)
SELECT
